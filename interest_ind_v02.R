source("~/Desktop/ShinyApps/int_ind/config_v02.R")
...
dgs_new_list <- lapply(dgs_list, function(x) x%>% select(date,value))
dgs_df<-Reduce(function(...) full_join(..., by = c('date')), dgs_new_list)
colnames(dgs_df)<-c("Date",yield_curve)
dgs_df_clean<-dgs_df[!is.na(dgs_df$DGS1),]
#dgs_df_clean<-dgs_df_clean[1:6558,]

#Read from tsy
tsy_5yr<-fread(name_file_map[[grep("tsy_5yr", name_file_map)]][2]) 
tsy_10yr<-fread(name_file_map[[grep("tsy_10yr", name_file_map)]][2])
tsy_30yr<-fread(name_file_map[[grep("tsy_30yr", name_file_map)]][2])
...
for(i in 1: (length(yield_curve)-1)){
  for(j in 1:(length(yield_curve)-i)){
    usty_df[paste0('spread_',yield_curve[i],"_",yield_curve[i+j])]<-usty_df[yield_curve[i]]-usty_df[yield_curve[i+j]]
  }
}
...
#################
for(model_index in 1:length(all_models_specs)){
model_id<-all_models_specs[[model_index]][[1]][1]
tree_size<-all_models_specs[[grep(model_id, all_models_specs)]][[2]][1]
weight1<-all_models_specs[[grep(model_id, all_models_specs)]][[2]][2]
weight2<-all_models_specs[[grep(model_id, all_models_specs)]][[2]][3]
weight3<-all_models_specs[[grep(model_id, all_models_specs)]][[2]][4]
#######################################
...
write.csv(result_df,paste0(out_dir,out_file_sfx,model_id,out_file_ext))
}
##########################################################
